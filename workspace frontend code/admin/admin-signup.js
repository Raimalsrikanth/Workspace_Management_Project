...
//checking phone number having 10 digits or not
   ...
   //!do_number sholud not be empty 
    ...
    if(pincode.value===''){
     ...
     span[10].style.visibility="hidden"
      ...
      if(admin_name.value !="" && email.value!="" && pwd.value !="" && phone.value !=""  && do_number.value !="" && landmark !=""&&street.value !="" &&city.value && district.value !="" && state.value !=""&& pincode.value !="" &&ele.value!=""){
        const admin={
       ...
       const response=await fetch("http://localhost:8080/saveadmin",
            ...
            if(response.ok){
              ...
              console.log(data);
              window.alert("Congratulations you have successfully created a new Admin Account 🥳🥳🥳");
              
              function o() {
                window.open(URL="./client/client-sign-in.html","_self");
              }
            ...
            if (response.status >= 400) {
              window.alert("Your having Already Registered Here Please Try To Login 😕😕😕");
